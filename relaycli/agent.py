...
- **Self-correction**: if parsing fails, auto-builds a repair prompt (max 1
  retry) back to the LLM.
- **Async I/O**: tool execution uses ``asyncio.gather`` for concurrent
  independent tool calls.
- **Heuristics**: hardcoded keyword lists extracted to ``heuristics.yaml``.
- **Smart trim**: ``Session.trim()`` preserves last 3 messages and uses
...
class Agent:
    ...
    def __init__(
        ...
        *,
        ...
        from relaycli.memory import memory_prompt_block
    ...
    def run(self, request: str, *, reporter: Reporter | None = None) -> AgentResult:
        ...
        for i in range(1, self.settings.max_iterations + 1):
            ...
            if not response.tool_calls:
                ...
                # Self-correction: if text_tool_calls returned nothing but the
                # text looks like it *might* contain JSON, try json-repair and
                # re-parse once.
            ...
            if len(active_tool_calls) > 1 and _are_independent(active_tool_calls):
                try:
                ...
                for call, (content, result) in zip(active_tool_calls, results):
                    ...
                    _update_file_counters(call, result, file_write_successes)
            else:
...
def _json_from_text(text: str) -> object | None:
    """Extract JSON from text using ``json_repair`` as the primary strategy.

    Falls back to bracket-matching (``_first_json_blob``) if repair fails.
    """
    raw = (text or "").strip()
    if not raw:
        return None
    ...
    # Strategy 1: try json_repair on the whole text.
    repaired = repair_json(raw)
    if repaired:
        candidates.append(repaired)

    # Strategy 2: try json_repair on code-fenced blocks.
    import re as _re
    ...
    # Strategy 3: bracket-matching as final fallback.
...
def _likely_json_attempt(text: str) -> bool:
    """Heuristic: does the text look like it's trying to emit JSON?"""
...
def _json_repair_nudge(count: int) -> str:
    plural = "s" if count > 1 else ""
    return (
        f"Your previous response contained malformed JSON tool call{plural} "
        "that RelayCLI attempted to repair, but the repaired version still "
        "could not be parsed. Please respond with valid JSON for the available "
        "tools only. Use the exact tool names and argument types from the "
        "tool list in the system prompt. If you cannot emit tool calls, "
        "provide a plain-text final answer."
    )


# ── Text-tool extraction (uses heuristics) ────────────────────────────────
...
def _tool_name_and_arguments(payload: dict) -> tuple[str | None, object]:
    ...
    if isinstance(function, dict):
        source = function
    name = next(
        (source.get(key) for key in ("name", "tool", "tool_name", "function_name", "action")
         if isinstance(source.get(key), str)),
        None,
    )
    arguments = next(
        (source[key] for key in ("arguments", "args", "parameters", "params", "input")
         if key in source),
        _MISSING,
    )
    if arguments is _MISSING and source is not payload:
        arguments = next(
            (payload[key] for key in ("arguments", "args", "parameters", "params", "input")
             if key in payload),
            _MISSING,
        )
    if arguments is _MISSING:
        ignored = {"name", "tool", "tool_name", "function_name", "action",
                    "function", "type", "id"}
...
def text_tool_call(
    ...
    *,
    call_id: str = "text_call_1",
    heuristics=None,
) -> ToolCall | None:
    calls = text_tool_calls(text, registry, call_id_prefix=call_id, heuristics=heuristics)
...
def text_tool_calls(
    ...
    *,
    ...
    Uses json-repair for robust JSON extraction and heuristics for
    alias resolution.
    """
    from relaycli.heuristics import load_heuristics
    h = heuristics or load_heuristics()
    ...
    for index, payload in enumerate(payloads, start=1):
        ...
        if not canonical:
            return []
        raw_arguments = _raw_tool_arguments(canonical, arguments)
...
def _are_independent(calls: list[ToolCall]) -> bool:
    """Heuristic: can these tool calls be run concurrently?

    Only read-only tools (read_file, list_dir, find_files, search) are
    considered safe for parallel execution. Write/edit/create tools must
...
def _update_file_counters(call: ToolCall, result: ToolResult | None, file_write_successes: int) -> int:
    if (
        call.name in {"write_file", "edit_file"}
        and result is not None
        and result.ok
    ):
        return file_write_successes + 1
    return file_write_successes


# ── Nudge messages (unchanged from original, using heuristics) ─────────────
