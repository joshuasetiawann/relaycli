...
Async support:
- :meth:`confirm_async` provides an async-compatible approval that delegates
  to :meth:`confirm` in a thread executor when stdin interaction is needed.
- :class:`AsyncPermissionManager` wraps the sync API for use in async contexts.
"""
...
class PermissionManager:
    ...
    async def confirm_async(self, action: str, *, prompt_text: str) -> Decision:
        """Async version of :meth:`confirm`.

        If a prompt is needed (non-auto), it runs in a thread executor so the
        async event loop is not blocked during user interaction.
        """
        if self.is_auto(action):
            return Decision(True, auto=True, reason=f"{self.mode} auto-approved {action}")
