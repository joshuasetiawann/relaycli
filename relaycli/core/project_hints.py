...
from relaycli.core.context import ALWAYS_IGNORE_DIRS, ProjectContext

_WEB_SUFFIXES = {".html", ".css", ".js", ".jsx", ".ts", ".tsx", ".vue", ".svelte"}
_WEB_FILENAMES = {"index.html", "styles.css", "style.css", "app.css", "app.js", "main.js", "script.js",
...
def project_prompt_block(project: ProjectContext) -> str:
    ...
    if web:
        ...
        lines.append("- If changing an existing website, read the matching files above first. Do not assume `src/index.html` unless it is listed.")
    else:
...
def missing_path_hint(project: ProjectContext, requested: str, *, limit: int = 8) -> str:
    ...
    candidates = [path for path in likely_web_files(project, limit=50) if Path(path).name.lower() == req_name]
    ...
    return f"\n\nExisting likely paths: {shown}. Use one of these exact relative paths, or run list_dir/find_files first."
...
def _iter_project_files(project: ProjectContext, *, max_depth: int, max_files: int) -> list[Path]:
    found: list[Path] = []
    root = project.root
