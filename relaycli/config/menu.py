...
from relaycli.config.manager import (
    PROVIDER_ENV, AppConfig, ProviderConfig, RoleConfig, effective_roles,
    load_app_config, mask_key, save_app_config, set_runtime_option,
)
...
class ConfigMenu:
    ...
    def render(self, console: Console) -> None:
        console.print(Panel(f"[bold {ACCENT}]Configuration[/bold {ACCENT}]   [dim]providers & keys · roles & models — separate from /settings[/dim]", border_style=ACCENT, expand=False))
        if self.section == "providers":
            self._render_providers(console)
        else:
            self._render_roles(console)
        console.print("[dim]› roles · providers · enable/disable <role> · model <role> <model|tier> · tier <t> <model> · key <provider> <env:VAR|value> · q[/dim]")
    ...
    def _render_roles(self, console: Console) -> None:
        ...
        for r in effective_roles(self.cfg):
            ...
            resolved = f"[red]{escape(r.error)}[/red]" if r.error else escape(str(r.model))
    ...
    def handle(self, line: str) -> tuple[str, bool]:
        ...
        if cmd == "help":
            return ("roles/providers switch view · enable/disable <role> · model <role> <model|tier> · tier <t> <model> · key <provider> <env:VAR|value> · q", False)
...
class SettingsMenu:
    ...
    def render(self, console: Console) -> None:
        console.print(Panel(f"[bold {ACCENT}]Settings[/bold {ACCENT}]   [dim]general preferences only — roles/models/keys live in /config[/dim]", border_style=ACCENT, expand=False))
        ...
        console.print(t)
        console.print("[dim]› mode <suggest|auto-edit|full-auto> · theme <name> · context <n> · q[/dim]")
